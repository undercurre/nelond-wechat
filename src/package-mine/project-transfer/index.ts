...
ComponentWithComputed({
  ...
  lifetimes: {
    ready: async function () {
      ...
      const res = await getShareId({ projectId: projectBinding.store.currentProjectId })
  ...
  methods: {
    ...
    getShareParams(params: { expire: number }) {
      ...
      return `type=transferHome&projectId=${projectBinding.store.currentProjectId}&expireTime=${expireTime}&shareId=${this.data._shareId}&userId=${userBinding.store.userInfo.userId}`
    },
    showQrCode() {
    ...
    toTransferHomeMember() {
      if (projectBinding.store.currentProjectDetail.userCount <= 1) {
        Toast('没有其他成员可供转让')
